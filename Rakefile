...
BUILD_DIR = "build"
BUILD_EXT_DIR = File.join(BUILD_DIR, "#{RbConfig::CONFIG['arch']}", 'subdomainz_c', RUBY_VERSION)

def gem_spec
  @gem_spec ||= Gem::Specification.load('subdomainz.gemspec')
end
...
if RUBY_PLATFORM == "java"
  ...
  task :specs => TEST_DEPS do
    sh %{#{Gem.ruby} -w -S rspec #{Dir["spec/subdomainz/*_spec.rb"].join(" ")} -fs --color}
  end
  desc "Run rubinius specs"
  task :rbxspecs => TEST_DEPS do
    sh %{#{Gem.ruby} -w -S rspec #{Dir["spec/subdomainz/rbx/*_spec.rb"].join(" ")} -fs --color}
  end
else
  TEST_DEPS.unshift :compile
  desc "Run all specs"
  task :specs => TEST_DEPS do
    ENV["MRI_SUBDOMAINZ"] = "1"
    sh %{#{Gem.ruby} -w -Ilib -I#{BUILD_EXT_DIR} -S rspec #{Dir["spec/subdomainz/*_spec.rb"].join(" ")} -fs --color}
  end
  desc "Run rubinius specs"
  task :rbxspecs => TEST_DEPS do
    ENV["MRI_SUBDOMAINZ"] = "1"
    sh %{#{Gem.ruby} -w -Ilib -I#{BUILD_EXT_DIR} -S rspec #{Dir["spec/subdomainz/rbx/*_spec.rb"].join(" ")} -fs --color}
  end
end
...
namespace :gem do
  task :install => :gem do
    ruby %{ -S gem install pkg/subdomainz-#{gem_spec.version}.gem }
  end
end
...
task :distclean => :clobber do
  ...
  FileUtils.rm_rf(Dir["lib/**/subdomainz_c.#{RbConfig::CONFIG['DLEXT']}"])
  FileUtils.rm_rf('lib/1.8')
  FileUtils.rm_rf('lib/1.9')
  FileUtils.rm_rf('lib/subdomainz/types.conf')
...
if USE_RAKE_COMPILER
  Rake::ExtensionTask.new('subdomainz_c', gem_spec) do |ext|
    ext.name = 'subdomainz_c'                                        # indicate the name of the extension.
